                ...
                script {
                    ...
                    dir('backend') {
                        ...
                        withCredentials([
                            ...
                            sh 'docker run -d --name ' + containerName + ' --network charsitynetwork -u root -e DATABASE_URI="$DATABASE_URI" -e NODE_ENV="$NODE_ENV" -v /var/run/docker.sock:/var/run/docker.sock -v jenkins-data:/var/jenkins_home -v $HOME:/home -v $WORKSPACE:/workspace -e VIRTUAL_HOST=api.wazpplabs.com -e VIRTUAL_PORT=3500 charsity-backend-test'

                            // Archive the test results generated by Mocha (JUnit XML format)
                            archiveArtifacts artifacts: 'test-results/test-results.xml', allowEmptyArchive: true
                        }
                    }
